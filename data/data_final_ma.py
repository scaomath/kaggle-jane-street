...
# %%
'''
data preparation (additional features) for the final submission (in order)
...
print(f"Number of running average features: {len(features_running)}")
# %%
try:
    ...
    with tqdm(total=len(train_debug)) as pbar:
        ...
        for _, row in train_debug.iterrows(): 
            ...
            for i, idx in enumerate(running_indices):
                feat_val[f'feature_{idx}_rm_500'] =  rm_500.get_mean()[i]
...
rm_500_df = pd.DataFrame(feat_vals_rm_500, index=train_debug.index)
rm_500_df.tail(7)
#%%

#%%
window_size = 500
rm = RunningMeanDay(window=window_size, num_feat=len(running_indices))
...
with tqdm(total=len(train)) as pbar:
    ...
    for _, row in train.iterrows(): 
        ...
        rm.push(x_tt[running_indices], date)

        feat_val = dict()
        for i, idx in enumerate(running_indices):
            feat_val[f'feature_{idx}_rm_{window_size}'] =  rm.get_mean()[i]
        feat_vals.append(feat_val)
        pbar.update()
#%%
feat_add_df = pd.DataFrame(feat_vals, index=train.index).astype(np.float32)
#%%
feat_add_df.to_parquet(os.path.join(DATA_DIR, f'feat_rm_{window_size}.parquet'), index=False)
# %%
